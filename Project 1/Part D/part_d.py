...
if __name__ == "__main__":
    ...
    X_train = np.stack(data_train_x)
    y_train = np.stack(data_train_y)

    X_test = np.stack(data_test_x)
    y_test = np.stack(data_test_y)

    # start algorithm
    aln_clf = ADAptiveLInearNEuron(eta=0.01, n_iter=5000)
    ...
    heatmap = sns.heatmap(cf_matrix, annot=True, cmap='Blues', fmt='g', xticklabels=np.unique(y_test),
